        ...
        <div className="min-h-screen bg-gradient-to-b from-navy-50 to-white">
            <section className="relative h-72 md:h-96 flex items-end overflow-hidden">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 w-full">
                    <span className="inline-block px-3 py-1 bg-teal-500/80 text-white text-xs font-bold uppercase tracking-widest rounded-full mb-3 backdrop-blur-sm">Semi-Luxury Rooms</span>
                    <h1 className="text-4xl md:text-6xl font-bold text-white leading-tight">
                        Semi-Luxury{' '}
                        <span className="bg-gradient-to-r from-teal-300 to-cyan-300 bg-clip-text text-transparent italic">Rooms</span>
                    </h1>
                    <p className="text-white/80 mt-2 text-lg max-w-xl">Premium comfort with stylish touches — beautiful stays at great value.</p>
                </div>
            </section>
                    ...
                    <div className="flex flex-col sm:flex-row items-end gap-4 flex-wrap">
                        <div>
                            ...
                            <input
                                ...
                                className="border border-navy-200 rounded-xl px-4 py-2 text-navy-800 font-semibold focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400 bg-navy-50 text-sm w-full sm:w-auto"
                            />
                        </div>
                        {selectedPackage !== 'day-use' && (
                            <div>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="lg:w-3/5 space-y-6">
                        ...
                        <p className="text-navy-500 text-sm mb-6">Click a room to preview facilities and package details.</p>
                        ...
                        {!loading && !error && rooms.map((room, idx) => (
                            <div key={room._id} onClick={() => handleSelectRoom(room)}
                                className={`group relative bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer border-2 transform hover:-translate-y-1 ${selectedRoom?._id === room._id ? 'border-teal-500 shadow-teal-100 shadow-2xl scale-[1.01]' : 'border-transparent hover:border-teal-200'}`}
                                style={{ animationDelay: `${idx * 120}ms` }}>
                                {selectedRoom?._id === room._id && (
                                    <div className="absolute top-4 right-4 z-20 bg-teal-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow">Selected</div>
                                )}
                                <div className="flex flex-col sm:flex-row">
                        ...
                        <div className="sticky top-28">
                            {selectedRoom ? (
                                ...
                                <div className="bg-white rounded-3xl shadow-lg border border-dashed border-navy-200 p-10 text-center">
                                    ...
                                    <p className="text-navy-400 text-sm">Select dates then choose a room to see full details.</p>
                                </div>
                            )}
                        </div>
                    </div>
